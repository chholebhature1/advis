...
type TalkToAiMessage = {
  role: "user" | "assistant";
  content: string;
  sentAt: string;
  insight?: TalkToAiInsight | null;
};

type TalkToAiMode = "planner" | "tax" | "risk";

type TalkToAiModeConfig = {
  label: string;
  subtitle: string;
  risk: string;
  accent: string;
...
type TalkToAiConversationStep = "collecting_goal" | "collecting_profile";

const TALK_TO_AI_MODE_ORDER: TalkToAiMode[] = ["planner", "tax", "risk"];

const TALK_TO_AI_MODE_CONFIG: Record<TalkToAiMode, TalkToAiModeConfig> = {
  planner: {
    label: "Planner AI",
    subtitle: "Goal roadmap",
    risk: "moderate",
    accent: "#2b5cff",
    softTint: "#e9f0ff",
    inputPlaceholder: "Ask Planner AI: How do I allocate monthly surplus across goals?",
    quickPrompts: [
      "I want to buy a 20 lakh car in next 3 years.",
      "I want a 30 lakh home down payment in 4 years.",
      "I want to build a 1 crore corpus in 10 years.",
    ],
    preferences: ["Goal-first planning", "Monthly discipline", "Family financial stability"],
  },
  tax: {
    label: "Tax AI",
    ...
    accent: "#0aa38f",
    softTint: "#e7fbf6",
    inputPlaceholder: "Ask Tax AI: What should I do this month for 80C and tax efficiency?",
    quickPrompts: [
      "I want to save tax while planning a car purchase in 3 years.",
      "I want tax-efficient investing for my home goal in 4 years.",
      "I want to maximize 80C and still stay goal-focused.",
    ],
    preferences: ["Tax efficiency", "Checklist execution", "Avoid last-minute decisions"],
  },
  risk: {
    label: "Risk AI",
    subtitle: "Protect downside",
    risk: "conservative",
    accent: "#a05cff",
    softTint: "#f3ebff",
    inputPlaceholder: "Ask Risk AI: How do I reduce concentration and protect downside risk?",
    quickPrompts: [
      "I want to buy a 20 lakh car in 3 years with low risk.",
      "I want a safer plan for my home down payment goal.",
      "I want to invest monthly but avoid big downside risk.",
    ],
    preferences: ["Drawdown control", "Risk-first allocation", "Peace of mind investing"],
  },
};

const TALK_TO_AI_FALLBACK_INSIGHT: TalkToAiInsight = {
  recommendation: "Unable to generate advice right now.",
  reason: "Temporary issue connecting to Pravix AI.",
  risk_warning: "Please avoid acting on uncertain assumptions.",
  next_action: "Try again or book a call with a Pravix expert advisor.",
};

const TALK_TO_AI_PERSONALIZATION_QUESTIONS = [
...
function buildTalkToAiPersonalizationPrompt(goal: string): string {
  return [
    `Great goal: ${goal}`,
    "",
    "To give you a more personalized and useful plan, please share 3 quick details:",
    ...TALK_TO_AI_PERSONALIZATION_QUESTIONS.map((question, index) => `${index + 1}. ${question}`),
    "",
    "Reply in one message using 1, 2, 3 format.",
  ].join("\n");
}

function toInsightText(value: unknown): string | null {
...
function normalizeTalkToAiInsight(value: unknown): TalkToAiInsight | null {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return null;
  }

  const maybe = value as Record<string, unknown>;
  const recommendation = toInsightText(maybe.recommendation);
  const reason = toInsightText(maybe.reason);
  const riskWarning = toInsightText(maybe.risk_warning ?? maybe.riskWarning);
  const nextAction = toInsightText(maybe.next_action ?? maybe.nextAction);
...
function normalizeTalkToAiFollowUpQuestions(value: unknown): string[] {
...
function formatTalkToAiInsight(insight: TalkToAiInsight): string {
  return [
    `Recommendation: ${insight.recommendation}`,
    `Reason: ${insight.reason}`,
    `Risk warning: ${insight.risk_warning}`,
    `Next action: ${insight.next_action}`,
  ].join("\n\n");
}
...
const MARKET_POLL_INTERVAL_MS = 30_000; // 30s — market indices don't need sub-second refresh on a marketing homepage
...
export default function HomepageClient() {
  ...
  const [talkToAiInput, setTalkToAiInput] = useState("");
  const [talkToAiError, setTalkToAiError] = useState<string | null>(null);
  const [isTalkToAiSending, setIsTalkToAiSending] = useState(false);
  const [talkToAiMode, setTalkToAiMode] = useState<TalkToAiMode>("planner");
  const [talkToAiConversationStep, setTalkToAiConversationStep] = useState<TalkToAiConversationStep>("collecting_goal");
  const [talkToAiPendingGoal, setTalkToAiPendingGoal] = useState<string | null>(null);
  const [talkToAiSuggestedPrompts, setTalkToAiSuggestedPrompts] = useState<string[]>([]);
  const [talkToAiMessages, setTalkToAiMessages] = useState<TalkToAiMessage[]>(() => [
    {
      role: "assistant",
      content: "Hi, I am Pravix AI. Ask me about goals, risk, tax planning, or your monthly investing strategy.",
      sentAt: new Date().toISOString(),
      insight: null,
    },
  ]);
  const talkToAiMessageContainerRef = useRef<HTMLDivElement | null>(null);
  const activeTalkToAiMode = TALK_TO_AI_MODE_CONFIG[talkToAiMode];
  const talkToAiQuickPrompts = talkToAiConversationStep === "collecting_profile"
    ? [
        "Monthly salary: INR 1,20,000; expenses: INR 65,000",
        "Existing assets for goal: INR 4,50,000",
        "EMIs: INR 18,000; monthly investable: INR 25,000",
      ]
    : talkToAiSuggestedPrompts.length > 0
      ? talkToAiSuggestedPrompts
      : activeTalkToAiMode.quickPrompts;

  const talkToAiUserProfile = useMemo<TalkToAiUserProfile>(
    () => ({
      age: 31,
      goals: ["Home down payment in 4 years", "Retirement corpus growth", "Tax-efficient investing"],
      risk: activeTalkToAiMode.risk,
      preferences: [...activeTalkToAiMode.preferences, `AI mode: ${activeTalkToAiMode.label}`],
    }),
    [activeTalkToAiMode],
  );
  ...
  useEffect(() => {
    const container = talkToAiMessageContainerRef.current;
    if (!container) {
      return;
    }

    container.scrollTop = container.scrollHeight;
  }, [talkToAiMessages, isTalkToAiSending]);

  useEffect(() => {
    setTalkToAiError(null);
    setTalkToAiSuggestedPrompts([]);
  }, [talkToAiMode]);

  const sendTalkToAiMessage = useCallback(
    async (rawMessage: string) => {
      const question = rawMessage.trim();
      if (!question || isTalkToAiSending) {
        return;
      }

      const userMessage: TalkToAiMessage = {
        role: "user",
        content: question,
        sentAt: new Date().toISOString(),
      };

      setTalkToAiMessages((previous) => [...previous, userMessage]);
      setTalkToAiInput("");
      setTalkToAiError(null);

      if (talkToAiConversationStep === "collecting_goal") {
        setTalkToAiPendingGoal(question);
        setTalkToAiConversationStep("collecting_profile");
        setTalkToAiMessages((previous) => [
          ...previous,
          {
            role: "assistant",
            content: buildTalkToAiPersonalizationPrompt(question),
            sentAt: new Date().toISOString(),
            insight: null,
          },
        ]);
      ...
      const goalContext = talkToAiPendingGoal ?? "User wants practical financial planning guidance";
      const normalizedProfileAnswer = question.replace(/\s+/g, " ").trim();
      const enrichedQuestion = [
        `Primary user goal: ${goalContext}`,
        `Personal details shared by user: ${question}`,
        "Use these details to give more personalized guidance.",
      ].join("\n");

      const enrichedProfile: TalkToAiUserProfile = {
        ...talkToAiUserProfile,
        goals: [goalContext, ...talkToAiUserProfile.goals].slice(0, 8),
        preferences: [
          ...talkToAiUserProfile.preferences,
          `User financial details: ${normalizedProfileAnswer.slice(0, 220)}`,
        ].slice(0, 8),
      };

      setTalkToAiConversationStep("collecting_goal");
      setTalkToAiPendingGoal(null);
      setIsTalkToAiSending(true);
      ...
      try {
        ...
        const payload = (await response.json().catch(() => ({}))) as TalkToAiInsightResponse;

        if (!response.ok) {
          throw new Error(payload.error ?? "Could not connect to Pravix AI right now.");
        }

        const insight = normalizeTalkToAiInsight(payload.insight) ?? TALK_TO_AI_FALLBACK_INSIGHT;
        const followUpQuestions = normalizeTalkToAiFollowUpQuestions(payload.followUpQuestions);

        setTalkToAiSuggestedPrompts(
          followUpQuestions.length > 0 ? followUpQuestions : activeTalkToAiMode.quickPrompts,
        );

        if (payload.fallbackUsed) {
          setTalkToAiError("Pravix AI is connected but currently in fallback mode. Please re-try in a moment.");
        }
        ...
        setTalkToAiMessages((previous) => [
          ...
          {
            role: "assistant",
            content: formatTalkToAiInsight(insight),
            sentAt: new Date().toISOString(),
            insight,
          },
        ]);
      } catch (error) {
        const message = error instanceof Error ? error.message : "Could not connect to Pravix AI right now.";
        setTalkToAiError(message);
        setTalkToAiSuggestedPrompts(activeTalkToAiMode.quickPrompts);

        setTalkToAiConversationStep("collecting_profile");
        setTalkToAiPendingGoal(goalContext);

        setTalkToAiMessages((previous) => [
          ...previous,
          {
            role: "assistant",
            content: formatTalkToAiInsight(TALK_TO_AI_FALLBACK_INSIGHT),
            sentAt: new Date().toISOString(),
            insight: TALK_TO_AI_FALLBACK_INSIGHT,
          },
        ]);
      } finally {
        setIsTalkToAiSending(false);
      }
    },
    [activeTalkToAiMode.quickPrompts, isTalkToAiSending, talkToAiConversationStep, talkToAiPendingGoal, talkToAiUserProfile],
  );

  const talkToAiInputPlaceholder = talkToAiConversationStep === "collecting_profile"
    ? "Reply with salary, existing assets, and EMIs/monthly investable amount"
    : activeTalkToAiMode.inputPlaceholder;
  ...
  return (
    <>
      ...
      <div className={`flex min-h-screen flex-col bg-[linear-gradient(180deg,#f8fbff_0%,#f1f6ff_42%,#e8f0ff_100%)] transition-opacity duration-700 ${isHeroReady ? "opacity-100" : "opacity-0"}`}>
        {/* HERO SECTION */}
        <section className="relative overflow-hidden pt-[5.5rem] pb-10 sm:pt-24 md:pb-14 md:pt-28 lg:min-h-[100svh]">
          ...
          <div className="relative z-20 mx-auto flex w-full max-w-7xl flex-col items-center gap-6 px-4 md:gap-10 md:px-10 lg:min-h-[calc(100svh-7rem)] lg:flex-row lg:items-center lg:gap-6 lg:px-14 xl:gap-10">
            {/* Left: Hero Content */}
            <div className="relative z-20 mt-0 flex w-full max-w-[36rem] flex-1 flex-col items-center text-center sm:mt-0 lg:-mt-12 lg:items-center lg:text-center xl:-mt-16 2xl:-mt-20">
              <div className="mb-4 flex w-full justify-center">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[9.5px] font-bold uppercase tracking-[0.18em] text-blue-50 shadow-sm backdrop-blur-md sm:px-3.5 sm:py-1.5 sm:text-[10px]">
              ...
              <div className="group relative mb-5 flex w-full flex-col items-center justify-center overflow-hidden rounded-[1.4rem] border border-white/20 bg-gradient-to-b from-white/15 to-white/5 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-xl sm:p-6 md:mb-8 md:rounded-[2rem] md:p-10">
                ...
                <div className="relative z-10 flex w-full flex-col items-center justify-center">
                  <h1 className="m-0 flex w-full flex-col items-center justify-center text-center">
                    <span className="block w-full text-center text-[clamp(2.4rem,13vw,5.8rem)] font-bold leading-[0.88] tracking-tight text-white drop-shadow-[0_8px_16px_rgba(0,0,0,0.5)] transition-colors duration-300">
                      Pravix
                    </span>
                    <span className="mt-2 block w-full pl-1 text-center text-[clamp(0.6rem,2.6vw,1.05rem)] font-bold uppercase tracking-[0.2em] text-[#00e0ff] drop-shadow-[0_0_16px_rgba(0,224,255,0.78)] sm:mt-3 sm:tracking-[0.42em]">
                      Wealth Management
                    </span>
                  </h1>

                  <div className="mt-4 h-px w-2/3 max-w-[200px] bg-gradient-to-r from-transparent via-[#00e0ff]/60 to-transparent" />

                  <p className="mt-3 max-w-sm text-center text-[10.5px] font-medium uppercase leading-snug tracking-[0.04em] text-blue-50/90 sm:text-xs md:text-[13px]">
              ...
              <div className="flex w-full flex-col items-center text-center">
                <h2 className="mb-2.5 text-[1.2rem] font-bold leading-[1.18] tracking-tight text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)] sm:text-[1.45rem] md:text-[1.9rem]">
                  Powered by{" "}
                ...
                <p className="mb-4 max-w-[34rem] text-[13px] font-medium leading-[1.55] text-blue-100/90 drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] sm:mb-6 sm:text-[14px] md:text-base">
                  Tell Pravix your goals and income, and within minutes you get a monthly SIP-ready roadmap for child education, home, and retirement.
                </p>

                <div className="flex w-full flex-col items-center gap-3 sm:flex-row sm:justify-center">
                  <Link
                    href="/onboarding"
                    className="group flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2b5cff] px-6 py-4 text-[15px] font-bold text-white shadow-[0_8px_25px_rgba(43,92,255,0.42)] transition-all active:scale-[0.97] sm:w-auto sm:rounded-full sm:px-8"
                  >
                  ...
                  <Link
                    href="/onboarding"
                    className="group flex w-full items-center justify-center gap-2 rounded-2xl border border-white/50 bg-white/10 px-6 py-4 text-[14px] font-semibold text-white backdrop-blur-sm transition-all active:scale-[0.97] sm:w-auto sm:rounded-full sm:px-7"
                  >
                    Talk to Expert
                    <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                  </Link>
            ...
            {/* Right: Animated Phone Mockup — hidden on mobile to give hero copy full focus */}
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8, delay: 0.2 }}
              className="hidden lg:flex w-full max-w-[30rem] flex-1 justify-end xl:max-w-[34rem]"
            >
              <HeroPhoneMockup />
            </motion.div>
          </div>
        ...
        >
          ...
          <div className="relative mx-auto w-full max-w-7xl px-4 md:px-10 lg:px-14">
            {/* ── Section Header ── */}
            <motion.div
            ...
            >
              ...
              <h2 className="text-[clamp(1.6rem,5vw,3.2rem)] font-bold leading-[1.1] tracking-tight text-[#0a1930]">
                Built for people planning{" "}
                <span className="bg-[linear-gradient(120deg,#2b5cff,#0099ff)] bg-clip-text text-transparent">
                  real life goals
                </span>
              </h2>
              <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-[#50607d] md:text-base lg:text-lg">
            ...
            <motion.div
              className="mt-8 grid grid-cols-2 gap-3 md:mt-12 lg:grid-cols-4"
              initial="hidden"
              whileInView="show"
            ...
            >
              {[
                ...
                <motion.article
                  key={card.title}
                  className="group relative overflow-hidden rounded-[1.2rem] border border-white/80 bg-white/70 p-4 shadow-[0_8px_32px_rgba(43,92,255,0.06)] backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(43,92,255,0.12)] sm:p-6"
                ...
                >
                  ...
                  <h3 className="mt-3 text-[14px] font-bold text-[#0a1930] sm:text-lg">{card.title}</h3>
                  <p className="mt-1.5 text-[12px] leading-relaxed text-[#586987] sm:text-[13px]">{card.desc}</p>
                </motion.article>
              ))}
            </motion.div>

            {/* ── Main Content: Family Visual + Money Mistakes ── */}
            <div className="mt-8 grid gap-5 md:mt-14 lg:grid-cols-[1.15fr_0.85fr] lg:items-stretch">
              ...
              <motion.div
                ...
                custom={0}
              >
                <div className="relative aspect-[4/3] sm:aspect-[16/9] lg:aspect-auto lg:h-full lg:min-h-[420px]">
                  <Image
                  ...
                  <div className="absolute left-4 top-4 z-10 sm:left-6 sm:top-6">
                    <motion.div
                      className="rounded-2xl border border-white/40 bg-[rgba(10,25,48,0.55)] px-4 py-3 shadow-[0_10px_28px_rgba(0,0,0,0.28)] backdrop-blur-xl"
                      initial={{ opacity: 0, x: -20 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      transition={{ duration: 0.6, delay: 0.3 }}
                      viewport={{ once: true }}
                    >
                      <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/90">Families Served</p>
                      <p className="mt-0.5 text-2xl font-bold text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.35)]">2,500+</p>
                    </motion.div>
                  </div>
                  ...
                  <div className="absolute right-4 top-4 z-10 sm:right-6 sm:top-6">
                    <motion.div
                      ...
                      viewport={{ once: true }}
                    >
                      <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/90">Goals Tracked</p>
                      <p className="mt-0.5 text-2xl font-bold text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.35)]">8,200+</p>
                    </motion.div>
                  </div>
              ...
              >
                {/* Money Mistakes Card */}
                <div className="flex-1 rounded-[1.4rem] border border-[#d8e7ff] bg-white p-4 shadow-[0_16px_40px_rgba(43,92,255,0.08)] sm:p-6">
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            SECTION: TALK TO AI - Strategic Conversion Section
            ═══════════════════════════════════════════════════════════════════ */}
        <motion.section
          id="talk-to-ai"
          className="relative overflow-hidden py-12 md:py-24 lg:py-32"
          style={{ background: "linear-gradient(180deg, #fffdf6 0%, #f4fbff 34%, #eef8ff 66%, #f7f2ff 100%)" }}
          variants={sectionReveal}
          initial="hidden"
          whileInView="show"
          viewport={denseSectionViewport}
        >
          <div className="pointer-events-none absolute -left-24 top-8 h-[340px] w-[340px] rounded-full bg-[radial-gradient(circle,rgba(255,184,116,0.24),transparent_72%)]" />
          <div className="pointer-events-none absolute left-[32%] top-1/4 h-[260px] w-[260px] rounded-full bg-[radial-gradient(circle,rgba(112,153,255,0.16),transparent_72%)]" />
          <div className="pointer-events-none absolute -right-24 bottom-4 h-[320px] w-[320px] rounded-full bg-[radial-gradient(circle,rgba(118,206,255,0.20),transparent_72%)]" />

          <div className="relative mx-auto w-full max-w-7xl px-4 md:px-10 lg:px-14">
            <motion.div variants={chartCardReveal} custom={0} className="relative">
              <div className="relative h-full overflow-hidden rounded-[2rem] border border-[#cde1ff] bg-[linear-gradient(160deg,#ffffff_0%,#f3f8ff_45%,#eef8ff_76%,#f7f2ff_100%)] p-6 text-[#12345f] shadow-[0_28px_70px_rgba(60,108,204,0.16)] sm:p-7">
                <div className="pointer-events-none absolute -right-12 -top-12 h-36 w-36 rounded-full bg-[#7fd7ff]/28 blur-3xl" />
                <div className="pointer-events-none absolute -bottom-12 -left-12 h-36 w-36 rounded-full bg-[#ffcd8b]/28 blur-3xl" />

                <div className="relative inline-flex items-center gap-2 rounded-full border border-[#9ec6ff] bg-white/85 px-3 py-1.5 shadow-[0_4px_14px_rgba(43,92,255,0.12)] backdrop-blur-sm">
                  <MessageCircle className="h-3.5 w-3.5 text-[#2b5cff]" />
                  <span className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#2b5cff]">Talk To</span>
                  <span className="bg-[linear-gradient(120deg,#2b5cff,#00a8ff,#7f4dff)] bg-clip-text text-[11px] font-bold uppercase tracking-[0.18em] text-transparent">AI</span>
                </div>

                <h3 className="relative mt-4 text-2xl font-bold leading-tight sm:text-[1.75rem]">
                  Live <span className="bg-[linear-gradient(120deg,#2b5cff,#00a8ff,#7f4dff)] bg-clip-text text-transparent">Pravix AI</span> chat
                </h3>
                <p className="relative mt-3 text-sm leading-relaxed text-[#4d658d] sm:text-[15px]">
                  Choose an AI mode, ask in plain language, and get structured guidance in seconds.
                </p>

                {talkToAiConversationStep === "collecting_profile" ? (
                  <p className="relative mt-2 text-xs font-semibold text-[#3f5f8c]">
                    Personalization step active: share salary, existing assets, and monthly investable capacity for a better answer.
                  </p>
                ) : null}

                <div className="relative mt-4 grid gap-2 sm:grid-cols-3">
                  {TALK_TO_AI_MODE_ORDER.map((modeKey) => {
                    const modeConfig = TALK_TO_AI_MODE_CONFIG[modeKey];
                    const isActive = talkToAiMode === modeKey;

                    return (
                      <button
                        key={modeKey}
                        type="button"
                        onClick={() => setTalkToAiMode(modeKey)}
                        className="rounded-xl border px-3 py-2 text-left transition-all duration-200 hover:-translate-y-0.5"
                        style={{
                          borderColor: isActive ? `${modeConfig.accent}66` : "#d6e5ff",
                          background: isActive ? `linear-gradient(135deg, ${modeConfig.softTint}, #ffffff)` : "rgba(255,255,255,0.78)",
                          boxShadow: isActive ? "0 8px 20px rgba(43,92,255,0.12)" : "none",
                        }}
                      >
                        <p className="text-[11px] font-bold uppercase tracking-[0.12em]" style={{ color: modeConfig.accent }}>
                          {modeConfig.label}
                        </p>
                        <p className="mt-0.5 text-[11px] text-[#5f7396]">{modeConfig.subtitle}</p>
                      </button>
                    );
                  })}
                </div>

                <div className="relative mt-5 rounded-2xl border border-[#d6e5ff] bg-white/76 p-4 shadow-[0_10px_26px_rgba(43,92,255,0.09)] backdrop-blur-xl">
                  <div className="flex items-center justify-between rounded-xl border border-[#d9e9ff] bg-[linear-gradient(140deg,#ffffff,#f5fbff)] px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <span className="relative flex h-2.5 w-2.5">
                        <span className="absolute inline-flex h-full w-full animate-ping rounded-full opacity-80" style={{ backgroundColor: activeTalkToAiMode.accent }} />
                        <span className="relative inline-flex h-2.5 w-2.5 rounded-full" style={{ backgroundColor: activeTalkToAiMode.accent }} />
                      </span>
                      <p className="text-[10px] font-bold uppercase tracking-[0.14em]" style={{ color: activeTalkToAiMode.accent }}>Pravix AI Online</p>
                    </div>
                    <p className="text-[10px] font-medium text-[#60759a]">Mode: {activeTalkToAiMode.label}</p>
                  </div>

                  <div ref={talkToAiMessageContainerRef} className="mt-3 max-h-[320px] space-y-3 overflow-y-auto pr-1">
                    {talkToAiMessages.map((message, index) => (
                      <article
                        key={`${message.role}-${index}-${message.sentAt}`}
                        className={`rounded-xl px-3 py-2.5 text-sm ${
                          message.role === "assistant"
                            ? "mr-5 border border-[#d8e7ff] bg-white text-[#1a365f]"
                            : "ml-10 border text-[#18345e]"
                        }`}
                        style={
                          message.role === "user"
                            ? {
                                borderColor: `${activeTalkToAiMode.accent}55`,
                                background: `linear-gradient(140deg, ${activeTalkToAiMode.softTint}, #ffffff)`,
                              }
                            : undefined
                        }
                      >
                        {message.role === "assistant" && message.insight ? (
                          <div className="mt-0.5 grid grid-cols-2 gap-2">
                            <div className="rounded-lg border border-[#dbe9ff] bg-[#f7fbff] p-2.5 text-xs text-[#335276]">
                              <p className="font-semibold uppercase tracking-[0.08em]" style={{ color: activeTalkToAiMode.accent }}>Recommendation</p>
                              <p className="mt-0.5">{message.insight.recommendation}</p>
                            </div>
                            <div className="rounded-lg border border-[#dbe9ff] bg-[#f7fbff] p-2.5 text-xs text-[#335276]">
                              <p className="font-semibold uppercase tracking-[0.08em]" style={{ color: activeTalkToAiMode.accent }}>Reason</p>
                              <p className="mt-0.5">{message.insight.reason}</p>
                            </div>
                            <div className="rounded-lg border border-[#ffd7d7] bg-[#fff5f5] p-2.5 text-xs text-[#5c3c3c]">
                              <p className="font-semibold uppercase tracking-[0.08em]" style={{ color: activeTalkToAiMode.accent }}>Risk Warning</p>
                              <p className="mt-0.5">{message.insight.risk_warning}</p>
                            </div>
                            <div className="rounded-lg border border-[#dbe9ff] bg-[#f7fbff] p-2.5 text-xs text-[#335276]">
                              <p className="font-semibold uppercase tracking-[0.08em]" style={{ color: activeTalkToAiMode.accent }}>Next Action</p>
                              <p className="mt-0.5">{message.insight.next_action}</p>
                            </div>
                          </div>
                        ) : (
                          <p className="whitespace-pre-wrap leading-relaxed">{message.content}</p>
                        )}
                        <p className="mt-1 text-[10px] text-[#6d83a6]">{formatChatTime(message.sentAt)}</p>
                      </article>
                    ))}

                    {isTalkToAiSending ? (
                      <div className="mr-5 inline-flex items-center gap-2 rounded-xl border border-[#d6e6ff] bg-[#f7fbff] px-3 py-2 text-xs text-[#4f668b]">
                        <Loader2 className="h-3.5 w-3.5 animate-spin" style={{ color: activeTalkToAiMode.accent }} />
                        Pravix AI is thinking...
                      </div>
                    ) : null}
                  </div>

                  <div className="mt-3 flex flex-wrap gap-2">
                    {talkToAiQuickPrompts.map((prompt) => (
                      <button
                        key={prompt}
                        type="button"
                        disabled={isTalkToAiSending}
                        onClick={() => void sendTalkToAiMessage(prompt)}
                        className="rounded-full border px-3 py-1.5 text-xs font-semibold transition-all hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-70"
                        style={{
                          borderColor: `${activeTalkToAiMode.accent}4d`,
                          backgroundColor: activeTalkToAiMode.softTint,
                          color: "#18345e",
                        }}
                      >
                        {prompt}
                  ...
                  {talkToAiError ? (
                    <p className="mt-2 text-xs text-[#bf5f5f]">{talkToAiError}</p>
                  ) : null}
                  ...
                  <form
                    ...
                    onSubmit={(event) => {
                      ...
                      void sendTalkToAiMessage(talkToAiInput);
                    }}
                  >
                    <input
                      value={talkToAiInput}
                      onChange={(event) => setTalkToAiInput(event.target.value)}
                      disabled={isTalkToAiSending}
                      placeholder={talkToAiInputPlaceholder}
                      className="h-11 flex-1 rounded-xl border border-[#d3e4ff] bg-white px-3.5 text-sm text-[#12345f] placeholder:text-[#7b90b3] focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-70"
                      style={{
                        boxShadow: "inset 0 1px 0 rgba(255,255,255,0.8), 0 4px 12px rgba(43,92,255,0.06)",
                      }}
                    />
                    <button
                      type="submit"
                      disabled={isTalkToAiSending || talkToAiInput.trim().length === 0}
                      className="inline-flex h-11 w-11 items-center justify-center rounded-xl border text-white transition-colors disabled:cursor-not-allowed disabled:opacity-65"
                      style={{
                        borderColor: `${activeTalkToAiMode.accent}55`,
                        backgroundColor: activeTalkToAiMode.accent,
                      }}
                      aria-label="Send message to Pravix AI"
                    >
                      {isTalkToAiSending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                    </button>
                  </form>

                  <div className="mt-3 rounded-xl border border-[#d4e6ff] bg-[#f7fbff] px-3 py-2 text-[11px] text-[#4f678c]">
                    Educational guidance only. For final allocation and execution decisions, confirm with a Pravix expert advisor.
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <motion.section
          ...
          className="relative w-full overflow-hidden py-14 text-white md:py-24 lg:py-36"
        ...
        >
          ...
          <div className="relative mx-auto grid w-full max-w-7xl gap-6 px-4 md:px-10 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch lg:gap-14 lg:px-14">
            <motion.div variants={chartCardReveal} custom={0}>
              <div className="group relative h-full min-h-[260px] overflow-hidden rounded-[2rem] border border-white/15 shadow-[0_32px_80px_rgba(0,0,0,0.35)] md:min-h-[340px]">
                <video
                  autoPlay
                  loop
                  muted
        ...
        <motion.section
          id="how-it-works"
          className="relative overflow-hidden bg-white py-14 md:py-24 lg:py-36"
          variants={sectionReveal}
          initial="hidden"
          whileInView="show"
          viewport={denseSectionViewport}
        >
          ...
          <div className="relative mx-auto w-full max-w-7xl px-4 md:px-10 lg:px-14">
            ...
            <motion.div className="mt-8 grid gap-4 sm:grid-cols-2 md:mt-12 lg:grid-cols-4" initial="hidden" whileInView="show" viewport={denseSectionViewport}>
              {[
                {
                  step: "01",
                ...
                <motion.article
                  key={item.step}
                  className="group relative overflow-hidden rounded-[1.4rem] border border-[#e2ebff] bg-white p-5 shadow-[0_8px_28px_rgba(43,92,255,0.06)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_50px_rgba(43,92,255,0.14)] sm:p-6"
                  variants={featureCardReveal}
                  custom={index}
                >
                  ...
                  <h3 className="mt-1 text-[15px] font-bold text-[#0a1930] sm:text-lg">{item.title}</h3>
            ...
            <motion.div
              className="mt-8 overflow-hidden rounded-[2rem] border border-[#2b5cff]/15 shadow-[0_32px_80px_rgba(10,25,48,0.2)] md:mt-14"
              style={{ background: "linear-gradient(155deg, #1d4494 0%, #2650a6 50%, #1e3f85 100%)" }}
              variants={chartCardReveal}
              custom={0}
            >
              <div className="relative p-6 text-white sm:p-8 md:p-12">
        ...
        <motion.section
          id="insights"
          className="relative overflow-hidden bg-[linear-gradient(160deg,#214a98_0%,#24539f_54%,#1f468d_100%)] py-12 text-white md:py-20 lg:py-28"
          variants={sectionReveal}
          initial="hidden"
          whileInView="show"
          viewport={sectionViewport}
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            SECTION: PRAVIX TEAM — Premium Redesign
            ═══════════════════════════════════════════════════════════════════ */}
        <motion.section
          id="pravix-team"
          className="relative overflow-hidden bg-white py-14 md:py-24 lg:py-36"
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              <div>
                <div className="mb-4 inline-flex items-center gap-2.5 rounded-full border border-[#2b5cff]/12 bg-[#edf4ff] px-4 py-1.5">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#2b5cff]">Pravix Team</span>
                </div>
        ...
        <motion.section
          id="blog"
          className="relative overflow-hidden py-14 md:py-24 lg:py-36"
        ...
        >
          ...
          <div className="relative mx-auto w-full max-w-7xl px-4 md:px-10 lg:px-14">
            ...
            <motion.div className="mt-6 grid gap-4 sm:grid-cols-2 md:mt-8 xl:grid-cols-4" initial="hidden" whileInView="show" viewport={denseSectionViewport}>
              {featuredBlogPosts.map((post, index) => (
                ...
                >
                  ...
                  <div className="p-4 sm:p-5">
                    ...
                    <h3 className="mt-2 line-clamp-2 text-[15px] font-bold text-[#0a1930] transition-colors group-hover:text-[#2b5cff]">{post.title}</h3>
        ...
        <div
          ...
          style={{ paddingBottom: "max(16px, env(safe-area-inset-bottom, 16px))", paddingLeft: "16px", paddingRight: "16px", paddingTop: "12px", background: "linear-gradient(to top, rgba(247,249,255,0.98) 60%, transparent)", backdropFilter: "blur(8px)" }}
        >
          <Link
            href="/onboarding"
            className="flex h-[52px] w-full items-center justify-center gap-2 rounded-2xl bg-[#2b5cff] text-[15px] font-bold text-white shadow-[0_8px_24px_rgba(43,92,255,0.38)]"
          >
