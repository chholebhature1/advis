...
import AgentAdvisorPanel, { type AiInsightUserProfile } from "@/components/AgentAdvisorPanel";
...
const aiInsightProfile = useMemo<AiInsightUserProfile>(() => {
  if (!profile) {
    return {
      age: null,
      goals: [],
  ...
  const goalSummary =
    `Build approximately INR ${Math.round(profile.target_amount_inr).toLocaleString("en-IN")}` +
    ` in ${profile.target_horizon_years} years`;

  const goals = [goalSummary];
  const notes = profile.notes.trim();
  if (notes.length > 0) {
    goals.push(notes.slice(0, 180));
  }
  ...
  return {
    ...
    preferences: [
      `tax_regime:${profile.tax_regime ?? "unspecified"}`,
      `contact:${profile.consent_to_contact ? "allowed" : "restricted"}`,
      `employment:${profile.employment_type ?? "unspecified"}`,
    ],
  };
}, [profile]);
...
const renderSignedInModule = (moduleKey: DashboardModuleKey, activeProfile: ProfileRow) => {
  ...
  return <AgentAdvisorPanel refreshKey={refreshTick} userProfile={aiInsightProfile} />;
};
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
                {kpiStripItems.map((item) => (
                  <button
                    ...
                    className={`rounded-xl border bg-white px-3 py-3.5 text-left shadow-[0_8px_18px_rgba(10,25,48,0.05)] transition-all hover:-translate-y-0.5 ${selectedKpiId === item.id ? "border-finance-accent ring-2 ring-finance-accent/20" : "border-finance-border"
                      }`}
                  >
                  ...
                  {orderedMarketIndicators.length === 0 ? (
                    ...
                    orderedMarketIndicators.map((indicator) => (
                      <div key={indicator.id} className="rounded-lg border border-finance-border bg-finance-surface/50 px-3 py-2.5">
                        ...
                        <p
                          className={`mt-1 text-[11px] ${indicator.trend === "up"
                              ? "text-finance-green"
                  ...
                  <article className="rounded-xl border border-finance-border bg-finance-surface/60 p-4">
                    ...
                    <div className="mt-3 space-y-2.5 text-sm">
                      ...
                      <div className="flex items-center justify-between gap-3 rounded-lg border border-finance-border bg-white px-3 py-2.5">
                        ...
                        <span
                          className={`font-semibold ${profileIntelligence && profileIntelligence.goalFundingStress <= 0
                              ? "text-finance-green"
                              : "text-amber-700"
                            }`}
                        >
